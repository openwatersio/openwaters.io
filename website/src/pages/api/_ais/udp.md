...
**Want credit for what you contribute?** Create a [personal token](#authentication) with `bind_ip: true` from the same address. The station then counts toward that token's contributor tier, which raises your stream limits and opens the raw NMEA feed.
